...
from shared.object import (
    Asset,
)  # I think this one works, but for some reason the IDE wasn't getting it
...
class ShotGridDatabase(Database):
    ...
    def get_assets(self, names: Iterable[str]) -> Set[Asset]:
        ...
        return set(
            Asset(os.path.basename(asset["sg_path"]), path=asset["sg_path"])
            for asset in assets
        )
    ...
    def get_asset_list(self) -> Sequence[str]:
        ...
        query = GetAllAssets(
            self, ["tags", "parents"]
        ).get()  # By default, this now filters out variants (child assets)
    ...
    def get_shot_list(self) -> Sequence[str]:
        ...
        for shot in query:
            shot_name = shot["code"]
            if (
                not "t" in shot_name.lower()
            ):  # This excludes the test shots, which begin 'T_0...'
...
class ShotGridQueryHelper(ABC):
    ...
    def __init__(
        ...
        base_filters.insert(
            ...
            [
                "project",
                "is",
                {"type": "Project", "id": self.shot_grid_database.PROJECT_ID},
            ],
    ...
    def _construct_all_fields(
        ...
        if override_fields:
            ...
            return list(
                set(self._create_base_fields() + additional_fields)
            )  # Remove duplicates
    ...
    def _get_code_name_filter(self, names: Iterable[str]) -> dict:
        return {
            ...
            "filters": [
                ["code", "is", name]
                for name in names  # You could also lowercase it, but in my tests it didn't matter
            ],
        }
...
class GetOneAssetByName(GetAllAssetsByName):
    def __init__(
        ...
        self.name = name
        super().__init__(
            database, [name], additional_fields, override_fields, filter_out_variants
        )
    ...
    def _get_one_asset_by_name(self, name: str):
        ...
        assert (
            len(assets) == 1
        ), f"Expected to find one asset with name {name}, but found {len(assets)}"
        return assets[0]
