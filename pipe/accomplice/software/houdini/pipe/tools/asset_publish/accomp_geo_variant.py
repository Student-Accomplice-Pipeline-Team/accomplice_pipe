...
class GeoVariant:
    ...
    def fix_internal_sop_outputs(node):
        ...
        for out_dict in [DEFAULT, PROXY]:
            GeoVariant.__verify_outnode(geo_network, out_dict)

    def get_asset_menu():
        # asset_names = pipe.server.get_asset_list()
        asset_names = data_cache.retrieve_from_cache(
            "asset_list", pipe.server.get_asset_list
        )
        menu_items = []
    ...
    def get_variant_menu():
        ...
        if asset_name is None or asset_name == "":
            return []
        asset = data_cache.retrieve_from_cache(
            asset_name, pipe.server.get_asset, asset_name
        )
    ...
    def get_save_names():
        ...
        geo_names = data_cache.retrieve_from_cache(
            "asset_list", pipe.server.get_asset_list
        )
        ...
        for name in geo_names:
            ...
            asset = data_cache.retrieve_from_cache(name, pipe.server.get_asset, name)
    ...
    def save():
        if hou.pwd().evalParm("save_path"):
            ...
            hou.ui.displayMessage(
                'No "Save Path" has been set.', severity=hou.severityType.Error
            )
        ...
        if asset_name:
            ...
            if response == 0:
                ...
                if response == 0:
                    ...
                    save_path = asset.get_geo_path() + variant_name + ".usdc"

                    if not os.path.exists(asset.get_geo_path()):
                        os.mkdir(asset.get_geo_path())

                    _, _, files = next(os.walk(asset.get_geo_path()))
                    for file in files:
            ...
            hou.ui.displayMessage(
                'No "Asset" has been set.', severity=hou.severityType.Error
            )
