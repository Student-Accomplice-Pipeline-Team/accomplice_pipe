...
class AnimationImporter:
    ...
    def get_character_options_list(self):
        ...
        if not os.path.isdir(anim_dir):  # Error check
            hou.ui.displayMessage(
                "There doesn't seem to be an anim folder in the context of this file. Contact pipe engineer."
            )
            return []
        ...
        for file in glob.iglob(
            path_to_check, recursive=True
        ):  # For each file in the anim directory
    ...
    def animation_name_update(self, node):
        ...
        anim_name = os.path.basename(anim_path).replace(
            EXTENSION, ""
        )  # Get just the name of the file, excluding extension
    ...
    def prune(self, node):
        ...
        if (node.parm("hidefxgeo").eval() == 1) and (
            node.parm("fx_enabled").eval() == 1
        ):
