...
class ImportLayout:
    ...
    ):  # Weirdly enough, 'node' apparently does't exist yet in the kwargs dictionary in the PythonModule, so some instances of this class will be created without a node passed in
        self.node = node
    ...
    def get_shot_menu(self):
        shot_names = data_cache.retrieve_from_cache(
            "shot_list", pipe.server.get_shot_list
        )
        menu_items = []

        for shot_name in shot_names:
            shot = data_cache.retrieve_from_cache(
                shot_name, pipe.server.get_shot, shot_name
            )
            ...
            if re.match(r"[A-Z]_[0-9][0-9][0-9A-Z]", current_shot_name):
                ...
                if "layout" in hou.hipFile.basename():
                    for index in range(previous_shot_index, -1, -1):
                        previous_shot = data_cache.retrieve_from_cache(
                            shot_names[index], pipe.server.get_shot, shot_names[index]
                        )
                        if os.path.isfile(previous_shot.get_layout_path()):
                            path = previous_shot.get_layout_path()
                            break
                else:
                    current_shot = data_cache.retrieve_from_cache(
                        current_shot_name, pipe.server.get_shot, current_shot_name
                    )
