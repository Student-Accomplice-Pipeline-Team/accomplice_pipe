...
class EditShader:
    ...
    def set_node_references(self, mat):
        self.materials[mat]["DiffuseColor"] = self.matLib().node(
            "diffuse_color_" + mat.name
        )
        self.materials[mat]["SpecularFaceColor"] = self.matLib().node(
            "spec_face_color_" + mat.name
        )
        self.materials[mat]["SpecularRoughness"] = self.matLib().node(
            "spec_roughness_" + mat.name
        )
        ...
        self.materials[mat]["Displacement"] = self.matLib().node(
            "displacement_" + mat.name
        )

        self.materials[mat]["BaseColor"] = self.matLib().node(
            "preview_diffuse_color_" + mat.name
        )
        self.materials[mat]["Metallic"] = self.matLib().node(
            "preview_metallic_" + mat.name
        )
        self.materials[mat]["Roughness"] = self.matLib().node(
            "preview_roughness_" + mat.name
        )
    ...
    def load_textures(self, material):
        ...
        for channel in channels:
            ...
            if channel == "SpecularRoughness":
                self.matLib().node("preview_roughness_" + material.name).parm(
                    "file"
    ...
    def assign_materials(self, value):
        ...
        if value == 1:
            ...
            for material in list(self.materials.keys()):
                component_mat.parm("primpattern" + str(count)).set(
                    "/ASSET/geo/render/" + material.name
                )
                component_mat.parm("matspecpath" + str(count)).set(
                    "/ASSET" + material.materialPath
                )

                component_mat.parm("primpattern" + str(count + 1)).set(
                    "/ASSET/geo/proxy/" + material.name
                )
                component_mat.parm("matspecpath" + str(count + 1)).set(
                    "/ASSET" + material.materialPath
                )
                ...
                if parm.name() != "outputnum":
                    if "iorCol" in parm.name():
                        if not hasIOR:
                            ...
                            out_parm = hou.FloatParmTemplate(
                                new_name,
                                parm.description(),
                                3,
                                default_value=[
                                    controls.evalParm("iorColr"),
                                    controls.evalParm("iorColg"),
                                    controls.evalParm("iorColb"),
                                ],
                        ...
                        if not hasSubSurfCol:
                            ...
                            out_parm = hou.FloatParmTemplate(
                                ...
                                3,
                                default_value=[
                                    controls.evalParm("sscr"),
                                    controls.evalParm("sscg"),
                                    controls.evalParm("sscb"),
                                ],
                                look=hou.parmLook.ColorSquare,
                                naming_scheme=hou.parmNamingScheme.RGBA,
                            )
                            ...
                            new_name = (
                                material.name + "_ssc" + parm.name().split("ssc")[1]
                            )
                            parm.setExpression('ch("../../' + new_name + '")')
                        ...
                        if not hasExtCoeff:
                            ...
                            out_parm = hou.FloatParmTemplate(
                                ...
                                3,
                                default_value=[
                                    controls.evalParm("extcoeffr"),
                                    controls.evalParm("extcoeffg"),
                                    controls.evalParm("extcoeffb"),
                                ],
                                look=hou.parmLook.ColorSquare,
                                naming_scheme=hou.parmNamingScheme.RGBA,
                            )
                        ...
                        if not hasFuzzCol:
                            ...
                            out_parm = hou.FloatParmTemplate(
                                ...
                                3,
                                default_value=[
                                    controls.evalParm("fuzzcolorr"),
                                    controls.evalParm("fuzzcolorg"),
                                    controls.evalParm("fuzzcolorb"),
                                ],
                                look=hou.parmLook.ColorSquare,
                                naming_scheme=hou.parmNamingScheme.RGBA,
                            )
                        ...
                        new_name = material.name + "_" + parm.name()
                        out_parm = hou.FloatParmTemplate(
                            new_name, parm.description(), 1, default_value=[parm.eval()]
                        )
                        out_parm.setMinValue(0)
