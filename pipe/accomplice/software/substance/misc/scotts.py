...
class OstrichTools():
    ...
    def buildUI(self) -> None:
        ...
        opacity_action = QtWidgets.QAction("Add opacity layer to current texture set", menu)
    ...
    def on_project_open(self, event: sp.event.Event) -> None:
        ...
        if envmap_old_url:
            ...
            sp.project.Metadata('OstrichTools').set('default_envmap', envmap_new.identifier().url())
        ...
        self.output_template = self.import_output_template(self.DEFAULT_OUTPUT_TEMPLATE, self.DEFAULT_TEMPLATE_NAME)

    def import_output_template(self, filename: str, template_name: Optional[str]) -> sp.resource.Resource:
        """Import an Output Template to the session"""
        print(f"Importing output template: { filename }")

        file_path = self.get_resource(filename)
        template = sp.resource.import_session_resource(file_path, sp.resource.Usage.EXPORT, template_name)
    ...
    def import_envmap(self, filename: str) -> sp.resource.Resource:
        ...
        return sp.resource.import_project_resource(envmap_path, sp.resource.Usage.ENVIRONMENT)
    ...
    def set_up_masks(self) -> None:
        if not sp.project.is_open():
            QtWidgets.QMessageBox.warning(None, "No project open", "Please open a project before setting up masks")
            return
        ...
        if not stack.has_channel(sp.textureset.ChannelType.User0):
            stack.add_channel(sp.textureset.ChannelType.User0, sp.textureset.ChannelFormat.L8, "Mask 1")
        if not stack.has_channel(sp.textureset.ChannelType.User1):
            stack.add_channel(sp.textureset.ChannelType.User1, sp.textureset.ChannelFormat.L8, "Mask 2")


    def set_up_opacity(self) -> None:
        if not sp.project.is_open():
            QtWidgets.QMessageBox.warning(None, "No project open", "Please open a project before setting up opacity")
            return
        ...
        if not stack.has_channel(sp.textureset.ChannelType.Opacity):
            stack.add_channel(sp.textureset.ChannelType.Opacity, sp.textureset.ChannelFormat.L8)
    ...
    def do_export(self) -> None:
        ...
        if not sp.project.is_open():
            QtWidgets.QMessageBox.warning(None, "No project open", "Please open a project before exporting textures")
            return
        ...
        if not error:
            QtWidgets.QMessageBox.information(None, "Export complete", "Textures exported successfully.")
