...
def launch_exporter():
    if not substance_painter.project.is_open():
        QtWidgets.QMessageBox.warning(
            None, "No project open", "Please open a project before trying to publish."
        )
        return
...
class SubstanceExporterWindow(QtWidgets.QMainWindow):
    ...
    def setup_UI(self):
        ...
        #####Text Sets#####
        self.texture_sets_dict = dict.fromkeys(
            substance_painter.textureset.all_texture_sets(), None
        )
    ...
    def do_export(self):
        ...
        RMAN_preset = substance_painter.resource.import_project_resource(
            os.path.join(resource_dir, "RMAN-ACCOMP.spexp"),
        ...
        PBRMR_preset = substance_painter.resource.import_project_resource(
            os.path.join(resource_dir, "PBRMR_ACCOMP.spexp"),
            substance_painter.resource.Usage.EXPORT,
        )
        ...
        for texture_set in self.texture_sets_dict:
            ...
            if error:
                QtWidgets.QMessageBox.warning(
                    self,
                    "Error",
                    "An error occurred while exporting textures. Please check the console for more information.",
                )
                return
        ...
        try:
            ...
            error = True
            print(e)
            QtWidgets.QMessageBox.warning(self, "Error", "Oh whoops, I screwed up")
        ...
        QtWidgets.QMessageBox.information(
            self, "Export complete", "Textures exported successfully."
        )
