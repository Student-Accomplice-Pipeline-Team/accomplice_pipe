...
class Asset(JsonSerializable):
    ...
    variants: Iterable[
        str
    ] = None  # TODO: It looks lik this is outdated or depreciated?
    id = None
...
class Material(JsonSerializable):
    ...
    def __init__(
        self, name: str, hasUDIMs: bool, isPxr=True, matType=MaterialType.BASIC.value
    ) -> None:
        self.name = name
    ...
    def from_string(in_str):
        ...
        for geovar in dct["hierarchy"]:
            ...
            for matvar in dct["hierarchy"][geovar]:
                materials = {}
                for material, data in dct["hierarchy"][geovar][matvar][
                    "materials"
                ].items():
                    mat = Material(
                        material, data["hasUDIMs"], data["isPxr"], data["matType"]
                    )
...
class Shot(JsonSerializable):
    ...
    def __init__(
        self, name: str, cut_in: Optional[int] = None, cut_out: Optional[int] = None
    ) -> None:
        self.name = name
    ...
    def get_shotfile_folder(self, department: Optional[str] = None) -> str:
        if department == "main" or department == None:
            ...
            raise ValueError(
                "type must be one of " + ", ".join(self.available_departments)
            )
        else:
