class DataCache:
    ...
    def retrieve_from_cache(self, key, loader_function=None, function_input=None):
        if key in self.cache:
            ...
            assert (
                loader_function is not None
            ), f"Key {key} not found in cache and no loader function was provided."
    ...
    def reload_cached_items(self):
        for key in self.cache:
            if key.endswith("function"):
                self.cache[key.replace("function", "")] = self.cache[key](
                    self.cache[key + "_function_input"]
                )
