...
def verify_shot_name(shot_name):
    ...
    assert shot_name in pipe.server.get_shot_list(), (
        "Shot name " + shot_name + " does not exist in the database."
    )
...
class FilePathUtils:
    ...
    def get_shot_name_from_file_path(file_path) -> str or None:
        ...
        path_split, shots_index = FilePathUtils._get_path_split_and_shots_index(
            file_path
        )
    ...
    def get_file_matching_substrings(
        folder_directory: str, substrings: list, enforce_only_one=True
    ) -> list or str:
        """
        ...
        if enforce_only_one:
            assert len(found_files) == 1, (
                "Found more than one file matching the given substrings: "
                + str(found_files)
            )
