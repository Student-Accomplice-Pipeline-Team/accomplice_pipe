...
def find_implementation(cls: type,
                        module: str,
                        package: Optional[str] = None) -> type:
    ...
    if importlib.util.find_spec(module, package):
        ...
        classes = getmembers(imported_module, lambda obj:
                             isclass(obj) and
                             not isabstract(obj) and
                             issubclass(obj, cls))

        # Check if more or less than one implementation was found
        if len(classes) < 1:
            raise AssertionError(f"module '{module}' does not contain an "
                                 f"implementation of class '{cls.__name__}'")
        elif len(classes) > 1:
            raise AssertionError(f"module '{module}' contains multiple "
