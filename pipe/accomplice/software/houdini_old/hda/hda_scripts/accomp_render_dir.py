...
def buildLayers(node):
    ...
    for i in range (1, node.parm("layer_count").eval() + 1):
        ...
        jsonSettings = None  
        if (layerName == 'beauty' and node.parm('low_quality_render' + str(i)).eval() == 1):
        ...
        if (node.parm('motion_blur' + str(i)).eval() == 0):
            mb_geo = node.createNode('rendergeometrysettings', node_name = 'MOTION_BLUR_rendergeometrysettings')
        ...
        exclude_geo.parm('primpattern1').set(node.parm('exclude_pattern' + str(i)).eval())
        exclude_geo.setInput(0, input, 0)
        input = exclude_geo
        #Build matte render geometry settings node
        mat_geo = node.createNode('rendergeometrysettings', node_name = 'MATTE_rendergeometrysettings')
        ...
        pha_geo = node.createNode('rendergeometrysettings', node_name = 'PHANTOM_rendergeometrysettings')
        pha_geo.parm('primpattern').set(node.parm('phantom_pattern' + str(i)).eval())
        pha_geo.parm('xn__primvarsriattributesvisibilitycamera_control_sdcckk').set('set')
        ...
        if layerName == 'beauty':
            ...
            rprop.parm('picture').set('$HIP/render/' + layerName + '/' + layerName + '.$F4.exr')
            #Make directory
        ...
        if layerName == 'beauty':
            name = node.parm('render_name' + str(i)).eval()
            usd_rop.parm('lopoutput').set('$HIP/renderUSD/' + name + '/' + name + '.$F4.usda')
        else:
            usd_rop.parm('lopoutput').set('$HIP/renderUSD/' + layerName + '/' + layerName + '.$F4.usda')
