    ...
    def check_if_selected(self):
        ...
        if len(curr_selection) == 0:
            confirm = cmds.confirmDialog(title='WARNING',
                    message='Nothing is selected', button=['Ok'],
...
# This GUI shows a list of the shots to select
    ...
    def shot_select_gui(self):
        ...
        cmds.button(label='Next', c=lambda x: \
    ...
    def get_frameRange_gui(self):
        ...
        self.window = cmds.window(windowID, title='Select Frame Range',
                                  sizeable=False, iconName='Short Name'
        ...
        cmds.rowLayout(nc=2)
        cmds.button(label='PlayBack range', command=lambda x: \
                    self.select_playback_range())
        cmds.button(label='Export', command=lambda x: \
                    self.get_frameRange())
        cmds.setParent('..')
    ...
    def get_frameRange(self):
        ...
        self.startFrame = cmds.textFieldGrp('startFrame', q=True,
                text=True)
        ...
        if not self.startFrame or not self.endFrame:
            ...
            error_message = \
                'End frame must be greater than the start frame'

        if error_message != '':
            confirm = cmds.confirmDialog(title='WARNING',
                    message=error_message, button=['Ok'],
                    defaultButton='Ok', dismissString='Other')
            if confirm == 'Ok':
                pass
        else:
    ...
    def version_camera_animation(self):
        ...
        camShape = cmds.listRelatives(newCam, children=True,
                fullPath=True, s=True)[0]  # Get a list of all children
        camChildren = cmds.listRelatives(newCam, children=True,
                fullPath=True)  # Get a list of all children
        ...
        try:
                mel.eval('file -force -options ";exportUVs=0;exportSkels=none;exportSkin=none;'
                         ...
                         + 'exportVisibility=1;mergeTransformAndShape=1;stripNamespaces=0" -typ "USD Export" -pr -eur -es "'+ version_path +'"')
        ...
        cmds.confirmDialog(title='Export Successful!',
                         message='Your camera has been exported to:'+ self.usd_filepath, button='Ok')
    ...
    def lockUnlockChannels(
        ...
        for y in range(len(sel)):
            for x in range(len(myChannels)):
                ...
                if keyable != '':
                    cmds.setAttr(sel[y] + myChannels[x], keyable=keyable,
                               channelBox=True)
                if hide != '':
                    cmds.setAttr(sel[y] + myChannels[x], keyable=hideVal)
                if breakConnections == True:
                    con = cmds.listConnections(sel[y] + myChannels[x],
                            s=1, p=1)
                    try:
                        cmds.disconnectAttr(con[0], sel[y]
                                + myChannels[x])
                    except:
                        pass
