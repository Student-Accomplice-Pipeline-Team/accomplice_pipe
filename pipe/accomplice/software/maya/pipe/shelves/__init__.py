...
def load():
    ...
    shelf_filepaths = [
        file
        for file in os.listdir(shelves_path)
        if file.startswith(SHELF_FILE_PREFIX) and file.endswith(SHELF_FILE_EXTENSION)
    ]
...
def _load_shelf(filename: str):
    ...
    cmds.scriptJob(
        event=["quitApplication", f"pipe.shelves._delete_shelf('{shelf_name}')"],
        permanent=True,
    )
